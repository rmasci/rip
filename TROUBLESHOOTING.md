...
## MergerFS Disk Errors or Corruption

**Problem:** Strange disk errors, file corruption, or I/O errors when using MergerFS storage pool
...
ls -la /mnt/disk1 /mnt/disk2 /mnt/disk3
```

### 2. Remount with Corrected Options

Unmount and remount with better settings for video files:

```bash
# Unmount the current pool
sudo umount /plex/storage

# Remount with improved options for large files
sudo mergerfs -o allow_other,use_ino,cache.files=partial,dropcacheonclose=true,category.create=mfs,category.action=mfs,category.search=mfs \
  /mnt/disk1:/mnt/disk2:/mnt/disk3 /plex/storage
```

**Key improvements:**
- `cache.files=partial` - Better than `off`, reduces I/O stress
- `category.create=mfs` - Creates files on disk with most free space
- `category.action=mfs` - All operations use most-free-space policy
- `category.search=mfs` - Searches all drives fairly

### 3. Update Persistent MergerFS Configuration

If you have MergerFS in `/etc/fstab`, update it:

```bash
# Edit fstab
sudo nano /etc/fstab

# Find the mergerfs line and replace it with:
/mnt/disk1:/mnt/disk2:/mnt/disk3 /plex/storage fuse.mergerfs allow_other,use_ino,cache.files=partial,dropcacheonclose=true,category.create=mfs,category.action=mfs,category.search=mfs 0 0
```
...
```bash
# Create a test file in the pool
dd if=/dev/zero of=/plex/storage/test.bin bs=1M count=100

# Check if it created successfully
ls -lh /plex/storage/test.bin

# Verify checksum integrity (optional)
md5sum /plex/storage/test.bin

# Clean up
rm /plex/storage/test.bin
```

### 5. Check Disk Health

If errors persist, check individual drive health:

```bash
# Check for bad blocks and errors
sudo smartctl -a /dev/sda
sudo smartctl -a /dev/sdb
sudo smartctl -a /dev/sdc

# Or use Ubuntu's disk utility GUI
gnome-disks
```

### 6. MergerFS Logs

Check system logs for MergerFS-related errors:

```bash
# View recent MergerFS errors
sudo journalctl -u mergerfs -n 50

# Or check kernel logs
sudo dmesg | grep -i mergerfs | tail -20
```

### 7. Permissions Issue

Ensure the user running `rip` has proper permissions on MergerFS:

```bash
# Check current permissions
ls -ld /plex/storage

# Fix permissions if needed (assuming user is 'rmasci')
sudo chown rmasci:rmasci /plex/storage
sudo chmod 755 /plex/storage
```

### Prevention Going Forward

- **Monitor drive health regularly** with `smartctl -a /dev/sda` etc.
- **Don't mix very different drive speeds** in the same pool
- **Keep MergerFS updated** - `sudo apt-get upgrade mergerfs`
- **Use UPS for your server** - Power interruptions can cause filesystem corruption
- **Run periodic filesystem checks**:
  ```bash
  sudo touch /forcefsck  # Forces fsck on next reboot
  sudo reboot
  ```
